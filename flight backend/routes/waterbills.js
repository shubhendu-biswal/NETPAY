...
router.post("/fetch", async (req, res) => {
  try {
    ...
    if (!bill) return res.json({ success: false, message: "No unpaid water bill found." });
...
router.get("/history/:consumerNumber", async (req, res) => {
  try {
    ...
    const bills = await WaterBill.find({ consumerNumber }).lean();
