...
router.post("/fetch", async (req, res) => {
  try {
    let { provider, consumerNumber } = req.body;

    if (!provider || !consumerNumber) {
      return res.json({ success: false, message: "Provider and consumer number are required." });
    }

    provider = provider.trim();
    consumerNumber = consumerNumber.trim();

    // Flexible match: case-insensitive provider, exact consumerNumber, only unpaid
...
router.post("/manual", async (req, res) => {
  try {
    const { provider, consumerNumber, areaCode, billAmount, dueDate, lateFee } = req.body;

    const newBill = new GasBill({
      provider,
      consumerNumber,
      areaCode,
      billAmount,
...
router.get("/history/:consumerNumber", async (req, res) => {
  try {
    ...
    const bills = await GasBill.find({ consumerNumber }).lean();
